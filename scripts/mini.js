...
if (subcmd === 'create') {
    try {
        ...
        util.puts('Error when handling create command: ' + err2);
        process.exit(1);
    }
    return;
}
...
fs.readFile(settingfile, function(err, data) {
    ...
    if (err) {
        util.puts('No module.json file found ('+err+').');
        process.exit(1);
    } else {
        try {
            settings = JSON.parse(data.toString('utf8', 0, data.length));
        } catch (err1) {
            util.puts('Error when parsing settings: ' + err1);
            process.exit(1);
        }
        try {
            cmd.handle(settings, mapping, _.rest(process.argv, 3));
        } catch (err2) {
            util.puts('Error when handling command: ' + err2);
            process.exit(1);
        }
    }
});
